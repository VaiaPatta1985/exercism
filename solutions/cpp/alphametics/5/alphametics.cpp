...
Icantbebothered solve(const std::string& equation) {
    ...
    auto check = [terms, sum](const std::array<int, 26>& combo) {return general_check(terms, sum, combo);};
#endif
...
#if SOLUTION == BRUTE_FORCE_RECONSTRUCT
bool general_check(const std::vector<std::string>& terms, const std::string& sum, const std::array<int, 26>& candidate) {

    auto c_sum = 0;
    for (const auto& term : terms) {
        std::string digit_term = "";
        for (auto letter : term) {
            digit_term += (char) candidate[(int)letter - 'A'] + '0';
        }
        c_sum += string_to_int(digit_term);
    }
    std::string digit_sum = "";
    for (auto letter : sum) {
        digit_sum += candidate[(int)letter - 'A'] + '0';
    }
