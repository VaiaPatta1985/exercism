...
public:
    ...
    public:
        ...
        bool operator==(binary_tree_iterator other) const {
            if (node_ptr == nullptr || other.is_end()) {
                return node_ptr == nullptr && other.is_end();
            }
            return **this == *other;
        };
    ...
    binary_tree(Node<T>* root, int unnecessary __attribute__((unused))) : m_root{root} {};
    ...
    std::unique_ptr<binary_tree<T>, std::default_delete<binary_tree<T>>> left() {
        if (m_root->left == nullptr) {
            return nullptr;
        }
        auto result = std::make_unique<binary_tree>(m_root->left, 0);
        return result;
    };

    std::unique_ptr<binary_tree<T>, std::default_delete<binary_tree<T>>> right() {
        if (m_root->right == nullptr) {
            return nullptr;
        }
        auto result = std::make_unique<binary_tree>(m_root->right, 0);
        return result;
    };
private:
    Node<T>* m_root;
