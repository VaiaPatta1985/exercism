...
std::array<Plants, 4> plants(const std::string& initials, const std::string& name) {
    const auto class_size = initials.length() / 2 + 1;
    ...
    if (offset == max_class_size) {
        throw (std::domain_error("This name does not belong to a student."));
    }
