...
std::map<char, int> frequency(const std::vector<std::string_view>& texts) {
    std::map<char, int> whole_alphabet_map = {
        {'a', 0},
        {'b', 0},
        {'c', 0},
        {'d', 0},
    ...
    auto stuff = std::vector<std::string_view>(texts);
    std::vector<std::thread> freq_counters;
    std::sort(stuff.begin(), stuff.end(), [](std::string_view a, std::string_view b){return a.length() < b.length();});
    std::vector<std::map<char, int>*> all_maps;
    int last_idx{};
    for (const auto& phrase : stuff) {
        all_maps.push_back(new std::map<char, int>(whole_alphabet_map));
    ...
    for (const auto& part_map : all_maps) {
        ...
        for (const auto& char_int_pair : partial_map) {
            ...
            whole_alphabet_map.at(letter) += freq;
        }
    }

    std::map<char, int> result = {};
    for (const auto& char_int_pair : whole_alphabet_map) {
        auto letter = char_int_pair.first;
        auto freq = char_int_pair.second;
        if (freq != 0) {
            result[letter] = freq;
