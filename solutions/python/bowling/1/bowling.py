	...
	def __update_score__(self):
		while self.frame_being_scored < 10:
			...
			if len(throws_to_score) == 1:
				try:
					if self.rolls[self.frame_being_scored + 1] == []:
						break
				except IndexError:
					break
			if throws_to_score == [10]:
				if len(self.rolls[self.frame_being_scored + 1]) < 2:
					try:
						if self.rolls[self.frame_being_scored + 2] == []:
							break
					except IndexError:
						break
					self.scores.append(10 + self.rolls[self.frame_being_scored + 1][0] + self.rolls[self.frame_being_scored + 2][0])
				else:
